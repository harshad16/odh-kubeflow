...
const (
	...
	// Timeout values used to wait until Route resources are available
	resourceRetryInterval = 2 * time.Second
	resourceRetryTimeout  = 15 * time.Second
)
...
func (r *OpenshiftNotebookReconciler) createOAuthClient(notebook *nbv1.Notebook, ctx context.Context) error {
	...
	oauthClientRoute, err := r.getRoute(notebook, ctx)
	if err != nil {
		log.Error(err, "Unable to retrieve route from OAuthClient", "route-name", notebook.Name)
		return err
	}
...
func (r *OpenshiftNotebookReconciler) getRoute(notebook *nbv1.Notebook, ctx context.Context) (*routev1.Route, error) {
	route := &routev1.Route{}
